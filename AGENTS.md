...
Source is split by layer: `backend/` houses the FastAPI ingestion and services (`backend/main.py`, `backend/models.py`, `backend/store.py`). Unit tests mirror the service tree under `backend/tests/`. The Next.js dashboard lives in `dashboard/`. CLI utilities such as `reddit_poller.py` are in `backend/` (or `scripts/` if moved). Docs (PRD, CLAUDE, AGENTS) stay in the root or `docs/`.

## Build, Test, and Development Commands
Install backend deps with `pip install -r backend/requirements.txt`, run the API via `uvicorn backend.main:app --reload` (from root) or `cd backend && uvicorn main:app --reload`. Start Reddit ingestion separately through `python -m backend.reddit_poller` (from root). Frontend work begins with `npm install --prefix dashboard`; `npm run dev --prefix dashboard` launches the dashboard and `npm run lint --prefix dashboard` enforces lint.
